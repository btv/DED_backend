...
def get_latest_update(cursor,file_path):
    query = """select blob from rawdata order by create_time desc limit 1 """
    blob = None
    try:
    ...
    with ZipFile(tmpFile,'r') as zip:
        data = str(zip.read(zip.filelist[0].filename)).split('"')
    if (len(data)>1):
        return data[1].split('\\n')
    return None
...
def main():
    ...
    try:
        ...
        cursor = conn.cursor()
        data = get_latest_update(cursor,data_file_path)
        cursor = conn.cursor()
        for line in data:
